    ...
    case "text":
      return (
        <div className="w-full min-h-[45dvh] p-4 text-left bg-[var(--color-surface-hover)]">
          <p className="text-sm whitespace-pre-wrap line-clamp-6">{content.body}</p>
        </div>
      );
    default:
      return null;
...
export default function FeedCard({ content, onClick }: Props) {
  return (
    <article className="snap-start bg-[var(--color-surface)] rounded-none sm:rounded-lg overflow-hidden border-y sm:border border-[var(--color-border)] min-h-[calc(100dvh-9.5rem)] sm:min-h-0">
      <div className="flex items-center gap-3 p-3">
        ...
        <Link to={`/profile/${content.creator_id}`} className="text-sm font-medium hover:text-indigo-400">
